...
filterByType(type)
{
  ...
  return Array.isArray(creatives) ? creatives.filter(item => item.act_as === type) : [];
}

renderLinks()
{
  return this.filterByType("link").map((item,i) => (<Link key={i} creative={item} />));
}

renderNewsletters()
{
  return this.filterByType("newsletter").map((item,i) => (<Newsletter key={i} creative={item} />));
}

renderImages()
{
  return this.filterByType("image").map((item,i) => (<Image key={i} creative={item} />));
}
...
<Card>
  ...
  <Heading first={true}>
    ...
    We recommend using promo materials provided below.
  </Heading>
  ...
  <Tabs>
    <Tab label="newsletters" onActive={handleActive} icon={<IconEmail />}>
