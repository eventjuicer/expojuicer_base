  ...
  render()
  {
    ...
    return (
      ...
      <Restricted authParams={{ foo: 'bar' }}>
        ...
        To participate in the <Link to={{pathname: "/ranking"}} >{translate('resources.ranking.name')}</Link> and be able to win rewards you must use dedicated tracking link.

        See <Link to={{pathname: "/creatives"}} >{translate('resources.creatives.name')}</Link>  for details.



        <p>{ getUserData('domain') } place: {sessions ? `#${position}` : `unknown`}</p>
